...
static void SetStatus(const wchar_t* t) { SetWindowTextW(g_hStatus, t); }
...
static bool IsElevated() {
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &tok)) {
        ...
        if (GetTokenInformation(tok, TokenElevation, &te, s, &s)) e = te.TokenIsElevated;
        CloseHandle(tok);
    }
    return e != FALSE;
}
    ...
    if (Process32FirstW(snap, &pe)) {
        do {
            ...
            EnumWindows([](HWND h, LPARAM lp) -> BOOL {
                auto* c = (Ctx*)lp; DWORD pid = 0;
                GetWindowThreadProcessId(h, &pid);
                if (pid == c->pid && IsWindowVisible(h)) { *c->out = h; return FALSE; }
                return TRUE;
            }, (LPARAM)&ctx);
...
static void RefreshProcessList() {
    ...
    DWORD prevPid = (prevSel >= 0 && prevSel < (int)g_pids.size()) ? g_pids[prevSel] : 0;
...
static bool IsModuleLoaded(DWORD pid, const std::wstring& name) {
    HANDLE s = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, pid);
    if (s == INVALID_HANDLE_VALUE) return false;
    MODULEENTRY32W me{ sizeof(me) }; bool f = false;
    if (Module32FirstW(s, &me)) do {
        if (!_wcsicmp(me.szModule, name.c_str())) { f = true; break; }
...
static bool EjectDLL(DWORD pid, const std::wstring& name) {
    HANDLE h = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid);
    if (!h) return false;
    HANDLE s = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, pid);
    ...
    uintptr_t base = 0;
    if (Module32FirstW(s, &me)) do {
        if (!_wcsicmp(me.szModule, name.c_str())) { base = (uintptr_t)me.modBaseAddr; break; }
    } while (Module32NextW(s, &me));
    CloseHandle(s);
    if (!base) { CloseHandle(h); return false; }
    auto freeLib = GetProcAddress(GetModuleHandleA("kernel32.dll"), "FreeLibrary");
...
static DWORD GetSelectedPid() {
    ...
    if (s < 0 || s >= (int)g_pids.size()) return 0;
    return g_pids[s];
}
static void OnInject(bool silent = false) {
    ...
    std::wstring name = fs::path(g_dllPath).filename().wstring();
    if (IsModuleLoaded(pid, name)) { if (!silent) Log(L"[=] Already injected in PID %lu", pid); return; }
...
static void OnEject() {
    ...
    std::wstring name = fs::path(g_dllPath).filename().wstring();
    if (!IsModuleLoaded(pid, name)) { Log(L"[=] Not injected."); return; }
    if (EjectDLL(pid, name)) { Log(L"[+] Ejected."); SetStatus(L"Ejected."); }
    else Log(L"[-] Eject failed.");
}
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM w, LPARAM l) {
    ...
    case WM_CREATE: {
        ...
        HWND t = CreateWindowW(L"STATIC", L"Muvixo Injector",
            WS_CHILD|WS_VISIBLE, 18, 14, 400, 24, hWnd, nullptr, hi, nullptr);
        SendMessageW(t, WM_SETFONT, (WPARAM)g_hFontBold, TRUE);
        ...
        HWND l1 = CreateWindowW(L"STATIC", L"Target process", WS_CHILD|WS_VISIBLE,
            18, 46, 200, 18, hWnd, nullptr, hi, nullptr);
        SendMessageW(l1, WM_SETFONT, (WPARAM)g_hFont, TRUE);
        ...
        HWND l2 = CreateWindowW(L"STATIC", L"DLL to inject", WS_CHILD|WS_VISIBLE,
            18, 258, 200, 18, hWnd, nullptr, hi, nullptr);
        SendMessageW(l2, WM_SETFONT, (WPARAM)g_hFont, TRUE);
        ...
        g_hBrowse = CreateWindowW(L"BUTTON", L"Browse...",
            WS_CHILD|WS_VISIBLE, 404, 278, 94, 26, hWnd, (HMENU)ID_BROWSE, hi, nullptr);
        SendMessageW(g_hBrowse, WM_SETFONT, (WPARAM)g_hFont, TRUE);

        g_hRefresh = CreateWindowW(L"BUTTON", L"Refresh",
            WS_CHILD|WS_VISIBLE, 18, 318, 100, 30, hWnd, (HMENU)ID_REFRESH, hi, nullptr);
        ...
        g_hEject = CreateWindowW(L"BUTTON", L"Eject",
            WS_CHILD|WS_VISIBLE, 422, 316, 76, 32, hWnd, (HMENU)ID_EJECT, hi, nullptr);
        SendMessageW(g_hEject, WM_SETFONT, (WPARAM)g_hFont, TRUE);

        HWND l3 = CreateWindowW(L"STATIC", L"Log", WS_CHILD|WS_VISIBLE,
            18, 358, 200, 18, hWnd, nullptr, hi, nullptr);
        SendMessageW(l3, WM_SETFONT, (WPARAM)g_hFont, TRUE);
    ...
    case WM_COMMAND: {
        ...
        case ID_AUTO:
            ...
            Log(g_auto ? L"[*] Auto-inject enabled." : L"[*] Auto-inject disabled.");
            break;
        case ID_PROC_LIST:
    ...
    case WM_TIMER:
        if (w == ID_TIMER_AUTO && g_auto) {
            ...
            if (pid && pid != g_lastInjectedPid) {
                std::wstring name = fs::path(g_dllPath).filename().wstring();
                Sleep(8000);
                if (!IsModuleLoaded(pid, name)) {
                    Log(L"[*] Auto-detected javaw.exe PID %lu", pid);
                    for (size_t i = 0; i < g_pids.size(); ++i)
                        if (g_pids[i] == pid) { SendMessageW(g_hList, LB_SETCURSEL, i, 0); break; }
                    OnInject(true);
                }
                g_lastInjectedPid = pid;
            }
            ...
            if (pid) {
                ...
                _snwprintf_s(buf, _countof(buf), _TRUNCATE,
                    IsModuleLoaded(pid, name) ? L"Injected (PID %lu)" : L"Ready (PID %lu)", pid);
                SetStatus(buf);
            } else SetStatus(L"No process selected");
        }
        return 0;
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int) {
    ...
    HWND hWnd = CreateWindowExW(0, wc.lpszClassName, L"Muvixo Injector — Alt Manager Edition",
        WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX,
        (sw-W)/2, (sh-H)/2, W, H, nullptr, nullptr, hInst, nullptr);
    if (!hWnd) return 1;
